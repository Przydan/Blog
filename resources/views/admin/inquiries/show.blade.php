    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white p-8 rounded-lg shadow border border-gray-200">
                <h1 class="text-2xl font-bold mb-6">Zapytanie od: {{ $inquiry->name }}</h1>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <p class="text-sm font-medium text-gray-500">Usługa</p>
                        <p class="text-lg text-gray-900">{{ $inquiry->service->title }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">E-mail</p>
                        <p class="text-lg text-gray-900">{{ $inquiry->email }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">Telefon</p>
                        <p class="text-lg text-gray-900">{{ $inquiry->phone }}</p>
                    </div>
                    <div class="col-span-2">
                        <p class="text-sm font-medium text-gray-500">Wiadomość</p>
                        <p class="text-lg text-gray-900 mt-2 bg-gray-50 p-4 rounded">{{ $inquiry->message }}</p>
                    </div>
                </div>
            </div>
            ...
            @if($inquiry->status === 'completed')
                <div class="bg-white p-8 rounded-lg shadow border border-gray-200">
                    ...
                    <form action="{{ route('admin.inquiries.store-response', $inquiry) }}" method="POST">
                        @csrf
                        <textarea name="message" rows="4" class="w-full border border-gray-300 rounded p-2 mb-2">{{ $inquiry->responses->first()?->message }}</textarea>
            ...
            <div class="bg-white p-8 rounded-lg shadow border border-gray-200">
                <h2 class="text-xl font-bold mb-4">Wewnętrzne notatki</h2>
                @foreach($inquiry->comments as $comment)
                    <div class="border-b border-gray-100 py-2">
                        <p class="text-sm">{{ $comment->comment }}</p>
                        <p class="text-xs text-gray-500">{{ $comment->user->name }} - {{ $comment->created_at->format('d.m.Y H:i') }}</p>
                    </div>
                @endforeach
                <form action="{{ route('admin.inquiries.store-comment', $inquiry) }}" method="POST" class="mt-4">
                    @csrf
                    <textarea name="comment" rows="2" class="w-full border border-gray-300 rounded p-2 mb-2" placeholder="Dodaj notatkę..."></textarea>
