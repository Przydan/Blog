<x-admin-layout>
    <div class="max-w-2xl mx-auto">
        ...
        <form method="POST" action="{{ route('admin.services.store') }}" class="bg-white p-8 rounded-lg shadow border border-gray-200 space-y-6">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="text" name="icon" id="icon" value="{{ old('icon') }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
                <label for="details" class="block text-sm font-medium text-gray-700 mb-1">Details (comma separated)</label>
                <input type="text" name="details" id="details" value="{{ old('details') }}" placeholder="CMS, E-commerce, Corporate Sites" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
