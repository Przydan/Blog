<x-admin-layout>
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold dark:text-white">Service Management</h1>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                ...
                <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                    @foreach($services as $service)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white dark:text-white">{{ $service->title }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $service->icon }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                <x-button variant="view" href="{{ route('admin.services.show', $service) }}" class="text-xs">{{ __("View") }}</x-button>
                                <x-button variant="edit" href="{{ route('admin.services.edit', $service) }}" class="text-xs">{{ __("Edit") }}</x-button>
