        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
            ...
            <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                @foreach($categories as $category)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white dark:text-white">{{ $category->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $category->slug }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">                                <x-button variant="view" href="{{ route('admin.categories.show', $category) }}" class="text-xs">{{ __("View") }}</x-button>
                                <x-button variant="edit" href="{{ route('admin.categories.edit', $category) }}" class="text-xs">{{ __("Edit") }}</x-button>
