        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
            <thead class="bg-gray-50 dark:bg-slate-700">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __("Technologies") }}</th>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                <x-button variant="view" href="{{ route('admin.portfolio.show', $project) }}" class="text-xs">{{ __("View") }}</x-button>
                                <x-button variant="edit" href="{{ route('admin.portfolio.edit', $project) }}" class="text-xs">{{ __("Edit") }}</x-button>
                                <form method="POST" action="{{ route('admin.portfolio.destroy', $project) }}" class="inline delete-form" data-confirm="{{ __("Are you sure you want to delete this") }} project?">
                                     @csrf
                                     @method('DELETE')
