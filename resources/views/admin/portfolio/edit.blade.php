    ...
    <div class="max-w-2xl mx-auto">
        ...
        <form method="POST" action="{{ route('admin.portfolio.update', $portfolio) }}" class="bg-white p-8 rounded-lg shadow border border-gray-200 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="technologies" id="technologies" value="{{ old('technologies', implode(', ', $portfolio->technologies ?? [])) }}" placeholder="Laravel, Tailwind, Vue.js" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div class="flex justify-end gap-4 pt-4">
                <form method="POST" action="{{ route('admin.portfolio.destroy', $portfolio) }}" class="inline delete-form" data-confirm="Are you sure you want to delete this project?">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-4 py-2 text-red-600 hover:text-red-800 transition font-medium">Delete Project</button>
                </form>
