                        ...
                        <div class="max-h-48 overflow-y-auto border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-md p-2 space-y-2">
                            @foreach($tags as $tag)
                                <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                                    <input type="checkbox" name="tags[]" value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', $post->tags->pluck('id')->toArray())) ? 'checked' : '' }} class="rounded border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white text-blue-600 shadow-sm focus:ring-blue-500">
                                    {{ $tag->name }}
                                </label>
                            @endforeach
                        </div>
